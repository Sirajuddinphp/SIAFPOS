...
import type { KotItem, KotPreview, KotStatus, KotTicketDetail, KotTicketSummary } from "../../shared/contracts/kot-contracts";
...
export class KotService {
  ...
  setPriority(kotUuid: string, priority: number): KotTicketDetail {
    if (![0, 1, 2].includes(priority)) {
      throw new KotError("INVALID_KOT_PRIORITY", "Priority must be 0, 1 or 2.");
    }
    const existing = this.kot.getDetail(kotUuid);
    if (!existing) throw new KotError("KOT_NOT_FOUND", "KOT not found.");
    if (existing.status === "completed") throw new KotError("KOT_ALREADY_COMPLETED", "Completed KOT cannot be changed.");
    this.kot.updatePriority(kotUuid, priority);
    const updated = this.kot.getDetail(kotUuid);
    if (!updated) throw new KotError("KOT_NOT_FOUND", "KOT not found.");
    return updated;
  }

  setStation(kotUuid: string, stationCode: string | null): KotTicketDetail {
    const existing = this.kot.getDetail(kotUuid);
    if (!existing) throw new KotError("KOT_NOT_FOUND", "KOT not found.");
    if (existing.status === "completed") throw new KotError("KOT_ALREADY_COMPLETED", "Completed KOT cannot be changed.");
    this.kot.updateStation(kotUuid, stationCode?.trim().toUpperCase() || null);
    const updated = this.kot.getDetail(kotUuid);
    if (!updated) throw new KotError("KOT_NOT_FOUND", "KOT not found.");
    return updated;
  }

  listActive(stationCode?: string | null): KotTicketSummary[] {
    return this.kot.listActiveTickets(stationCode?.trim().toUpperCase() || null);
  }
